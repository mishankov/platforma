...
`queue` package provides `Processor` struct manages a pool of workers to process jobs from a queue. It accepts `Handler` interface that handles job processing and `Provider` interface that handles interation with actual queue. `ChanQueue` is implementation of `Provider` interface that uses golang channels.
...
```go
func jobHandler(_ context.Context, job job) {
	log.Info("job handled", "data", job.data)
}
````
